-- Update send-support-inklings cron job to include secret token
-- 
-- ⚠️ SECURITY WARNING: This file contains a PLACEHOLDER for the secret.
-- DO NOT commit this file with a real secret value!
-- 
-- INSTRUCTIONS:
-- 1. Set SEND_SUPPORT_CRON_SECRET in Supabase Dashboard → Edge Functions → Secrets
-- 2. Copy this file and replace 'YOUR_SEND_SUPPORT_CRON_SECRET_HERE' with your actual secret
    ...
    SELECT
        net.http_post(
            ...
            headers := jsonb_build_object(
                ...
                'x-cron-secret', 'YOUR_SEND_SUPPORT_CRON_SECRET_HERE'  -- ⚠️ REPLACE WITH SECRET FROM DASHBOARD!
            ),
            body := '{}'::jsonb
        ) AS request_id;
    $$
...
SELECT 
    ...
    active
FROM cron.job
WHERE jobname = 'send-support-inklings';
