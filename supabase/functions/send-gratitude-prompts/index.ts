...
serve(async (req) => {
  ...
  try {
    ...
    // Only run during 2026 (January 1, 2026 to December 31, 2026)
    if (currentYear !== 2026) {
    ...
    // Check if it's the right time (16:00 UTC = 11:00 AM EST / 12:00 PM EDT)
    if (currentHour !== 16) {
    ...
    // Get all active gratitude challenge participants
    ...
    // Get the prompt for today's day of year
    const { data: prompt, error: promptError } = await supabase
      .from('prompt_bank')
      .select('id, prompt_text, category_name')
