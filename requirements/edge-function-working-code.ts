    ...
    for (const user of users) {
      try {
        ...
        const { data: rotationData, error: rotationError } = await supabase
          ...
          .select('*')
          .eq('uid', user.user_id)  // Fixed: use 'uid' instead of 'user_id'
          .single();
        ...
        if (timeDiff <= 45) {
          ...
          const categoryCountColumn = `prompt_count_${currentCategoryId.replace(/[^a-zA-Z0-9]/g, '_')}`;
            ...
            <body>
              <div class="container">
                ...
                <div class="footer">
                  ...
                  <a href="mailto:support@inklingsjournal.live" class="contact-link">Contact Support</a>
                </div>
              </div>
            </body>
            </html>
          ...
          const emailResponse = await fetch('https://api.resend.com/emails', {
            ...
            body: JSON.stringify({
              ...
              subject: `✍️ Your Ink-lings Journal Prompt`,
              html: emailHtml
            })
          });
          ...
          if (emailResponse.ok) {
            ...
            const { error: updateError } = await supabase.from('user_prompt_rotation').update({
              [categoryCountColumnForUpdate]: currentPromptCount + 1,  // Use the prompt we just sent + 1
